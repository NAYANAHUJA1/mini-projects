...
class chatbot:
    ...
    negres = ("no","nope","nah","naw","not a chance","sorry")
    exit = ("quit","pause","exit","goodbye","bye","later")
    ...
    def greet(self):
        ...
        if will_help in self.negres:
            print("have nice earth day!")
            return 
        self.chat()
        
    def doex(self, reply):
        for command in self.exit:
            if reply == command:
                print("have a nice day")
                return True

    def chat(self):
        reply = input(r.choice(self.revques)).lower()
        while not self.doex(reply):
